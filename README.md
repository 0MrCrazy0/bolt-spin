...
- **Bolt Token Premium** — played with ⚡ tokens only (1–50⚡ stake ladder; not a cash high-roller)
...
- **Player level ranks** — Rookie → Grinder  → Immortal..etc (Milestone levels unlock more machines)
...
- **Auto-spin controls** — limits, stop-loss floor, auto-clear win screen, feature popup auto-close
...
├── LICENSE         # MIT
├── CHEATS.md      # Admin/test codes (Konami on lobby)
└── README.md
```
...
| Option | Purpose |
|--------|---------|
| Sound / Music / Lobby music | SFX, machine themes, lobby bed + volume sliders |
| Haptics | Vibration on supported devices (Android; iOS has no vibrate API) |
| Spin speed | Real / Fast reel timing |
| Auto-spin limit | 10 / 40 / 100 / ∞ when Auto is on |
| Stop-loss floor | Stop Auto if balance hits the floor |
| Wheel auto-spin cap | Limit reward-wheel auto runs (OFF = no auto) |
| **Auto-clear win screen** | Only when **Auto Spin is OFF**. Clears base win overlay after total — does **not** spend a spin. With Auto Spin ON, totals auto-dismiss and play continues |
| **Feature popup auto-close** | Rare blocking feature dialogs auto-CONTINUE after ~2s |
