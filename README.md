...
Mobile-first **Hold & Spin** style slots in the browser.  
...
| | |
|:--|:--|
| **Cabinets** | **20** lobby regulars + **1** secret (🚽 Flush Fortune) + **1** Bolt Token Premium (⚡) = **22** |
...
1. **Lobby** — unlock machines by leveling up; badges show mastery (NEW → … → MYTHIC).
2. **Bet** — cash machines use $ bet × multi. **Bolt Token Premium** spends ⚡ (wins still pay **cash**).
3. **Base game** — line wins on the paylines; feature symbols and orbs can trigger bonuses. Rare **Scratch** mini-games (themed per machine; Flush = Wipe & Poop) — match 3 amounts, pays all three.
4. **6+ orbs** → **Hold & Spin** (respins while new orbs lock).
5. **3+ scatters** → **Free Games** (count depends on the machine; retriggers **stack**).
...
8. **Win totals** — after a big total, **Tap SPIN to continue** (or **AUTO · next spin shortly** when auto-spin is on). Totals soft-clear if you wait.

**High bet = high risk** — larger stakes pay more when they hit; cold runs can drain the bankroll. At very low balance the game gives a **slight** feature nudge so sessions can recover — not a guaranteed win.
