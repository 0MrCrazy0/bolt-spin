/**
 * Bolt Spin — Tone hybrid v9 (motif beds · clearer feature lifts)
...
(function(global){
  ...
  const THEMES={
    ...
    dragon:  {bpm:150, root:'D', bright:0.95,style:'march', bassEvery:3, seed:42},
    bolt:    {bpm:172, root:'C', bright:1.35,style:'pulse', bassEvery:2, seed:99},
    flush:   {bpm:118, root:'E', bright:0.55,style:'drone', bassEvery:4, seed:77}
  };

  const FEAT={
    base:  {bpmMul:1.0,  seedAdd:0,  style:null,   density:1, brightAdd:0},
    free:  {bpmMul:1.38, seedAdd:19, style:'spark', density:2, brightAdd:0.32},
    hold:  {bpmMul:1.52, seedAdd:37, style:'pulse', density:2, brightAdd:0.36},
    train: {bpmMul:1.62, seedAdd:53, style:'march', density:3, brightAdd:0.42}
  };
  ...
  function buildMelody(root, seed, len){
    ...
    const notes=[];
    // Motif anchors — more hooky than pure random walk
    const anchors=[2,4,5,7,4,5,7,9].map(i=>Math.min(scale.length-1,i));
    let idx=anchors[0];
    for(let i=0;i<len;i++){
      const r=rand();
      if(i%8===0) idx=anchors[(Math.floor(i/8)+seed)%anchors.length];
      else if(r<0.45) idx+=(rand()<0.55?-1:1);
      else if(r<0.7) idx+=(rand()<0.5?-2:2);
      else if(r<0.88) idx=anchors[Math.floor(rand()*anchors.length)];
  ...
  function applyVolumes(hot, bright){
    ...
    try{
      const master=-12+v*16+(hot?4.5:0);
      if(pad)pad.volume.value=master-1.5;
      if(bass)bass.volume.value=master-0.5;
      if(lead)lead.volume.value=master+2.5;
      if(sparkle)sparkle.volume.value=master+0.5;
      if(perc)perc.volume.value=master-5;
  ...
  function startBed(css, featKey){
    ...
    baseBpm=Math.round((th.bpm||128)*fi.bpmMul*(css==='lobby'?1.02:1.08));
  ...
  const api={
    ...
    winStinger(tier){
      ...
      try{
        ...
        n.forEach((note,i)=>{
          sparkle.triggerAttackRelease(note,'8n','+'+(i*0.055),0.78);
          lead.triggerAttackRelease(note,'8n','+'+(i*0.055),0.62);
        });
